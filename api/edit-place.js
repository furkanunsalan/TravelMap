...
export default async function handler(req, res) {
    if (req.method === 'PUT') {
        try {
            ...
            if (!updatedPlace.slug || !updatedPlace.name || !updatedPlace.latitude || !updatedPlace.longitude) {
                return res.status(400).json({ message: 'Missing required fields' });
            }

            // Query Firestore for the place with the matching slug
            const placeQuery = await db.collection('places').where('slug', '==', updatedPlace.slug).get();
